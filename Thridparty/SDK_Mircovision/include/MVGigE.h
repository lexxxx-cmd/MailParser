...
extern "C"
{
	...
	*/
	 ...
	 *  \retval  MVST_SUCCESS			: 成功
	 */
	MVGIGE_API MVSTATUS_CODES __stdcall MVGetOptimalPacketSize(HANDLE hCam, uint32_t* pPacketSize);
	 ...
	 *  \retval: MVST_SUCCESS			: 成功
	 */
	MVGIGE_API MVSTATUS_CODES __stdcall MVSetStreamHostPort(HANDLE hCam, unsigned int nPort);
	/*!
	 * \brief	读取网络数据包间隔。
	 * \param [in]	hCam				相机句柄
...
MVGIGE_API MVSTATUS_CODES __stdcall MVImageBayerToBGREx( HANDLE hCam,MV_IMAGE_INFO *pInfo,MVImage *pImage,double fGamma,bool bColorCorrect,int nContrast );
	 ...
	 *  \retval  	MVST_SUCCESS			: 成功
	 */
	MVGIGE_API MVSTATUS_CODES __stdcall MVGetSampleGrabBuf(HANDLE hCam,unsigned char *pImgBuf,unsigned long szBuf,int *pFrameID, int msTimeout);
